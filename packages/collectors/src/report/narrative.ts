...
export function generateReport(result: InvestigationResult): string {
  ...
  lines.push(`| Principle | Status | Evidence |`)
  lines.push(`|-----------|--------|----------|`)
  lines.push(`| 1. Data Integrity | ${acpo.dataIntegrity.compliant ? 'PASS' : 'FAIL'} | ${acpo.dataIntegrity.evidence} |`)
  lines.push(`| 2. Competence | ${acpo.competence.compliant ? 'PASS' : 'FAIL'} | ${acpo.competence.evidence} |`)
  lines.push(`| 3. Audit Trail | ${acpo.auditTrail.compliant ? 'PASS' : 'FAIL'} | ${acpo.auditTrail.evidence} |`)
  lines.push(`| 4. Overall Responsibility | ${acpo.overallResponsibility.compliant ? 'PASS' : 'FAIL'} | ${acpo.overallResponsibility.evidence} |`)
  lines.push('')
  lines.push(`**Overall ACPO compliance: ${acpo.allCompliant ? 'ALL PRINCIPLES MET' : 'NON-COMPLIANT'}**`)
  lines.push('')

  // legal framework
  lines.push(generateLegalBasis())
  ...
  lines.push('')
  lines.push('This report does not constitute legal advice. It provides a factual record of publicly observable data with mathematical analysis. Consult a solicitor before taking legal action based on these findings.')
  lines.push('')
  lines.push('---')
